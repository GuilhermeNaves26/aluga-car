...
from janelas.janela_exibir import ExibirVeiculo
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.btnCadastro = ctk.CTkButton(self.frameBtns, text='Cadastrar Veículo', width=187, height=183, image=self.img_cad,compound='top', fg_color="#363D50", font=('Open Sans', 16, 'bold'), command=self.cadastro)
        self.btnCadastro.pack(side = 'left', expand=True)

        self.btnExibir = ctk.CTkButton(self.frameBtns, text='Exibir Veículos', width=187, height=183, image=self.img_exibir, compound='top', fg_color='#363D50', font=('Opens Sans', 16, 'bold'), command=self.exibir)
        self.btnExibir.pack(side='left', expand= True)

        self.btnAlugar = ctk.CTkButton(self.frameBtns, text='Alugar Veículos', width=187, height=183, image=self.img_alugar, compound='top', fg_color='#363D50', font=('Opens Sans', 16, 'bold'), command=self.alugar)
    ...
    def exibir(self):
        janela_exibir = ExibirVeiculo(self)
